...
from .ACSettings import (
    AC_LOG_FILE_LOCATION, ALLOWED_CONTAINER_RETURN_STATUSES, ALLOWED_CONTAINER_TEST_TYPES,
    ALLOWED_CONTAINER_TEST_RESULTS, ALLOWED_CONTAINER_TEST_ACTIONS, INSIDE_CONTAINER_FILE_MOUNT,
    INSIDE_CONTAINER_FILE_DIRECTORY, CONTAINER_INPUT_CONFIG_FILE_NAME,
    CONTAINER_OUTPUT_PROGRESS_FILE, CONTAINER_OUTPUT_LOG_FILE, CONTAINER_REQUIRED_ENVIRONMENT_VARS,
    CONTAINER_RETURN_STATUS, CONTAINER_STATUS_ERROR, CONTAINER_TEST_DATA, CONTAINER_TEST_ACTION,
    CONTAINER_TEST_DATETIME, CONTAINER_TEST_LOGIC, CONTAINER_TEST_RESULT, CONTAINER_TEST_VALUE,
    CONTAINER_TEST_VALUE_TYPE
)
...
class ACBase(object):
        ...
        for var in CONTAINER_REQUIRED_ENVIRONMENT_VARS:
            ...
            if the_var_value is None:
                ACLogger.log_and_print_error(
                    "ACBase:  unable to open access env variable: %s" % var
                )
                self.valid_configuration = False
            self.env_vars[var] = the_var_value

        if self.valid_configuration:
            self.inside_progess_file = os.path.join(
                self.env_vars[INSIDE_CONTAINER_FILE_MOUNT],
                self.env_vars[INSIDE_CONTAINER_FILE_DIRECTORY],
                self.env_vars[CONTAINER_OUTPUT_PROGRESS_FILE],
            )

            self.inside_log_file = os.path.join(
                self.env_vars[INSIDE_CONTAINER_FILE_MOUNT],
                self.env_vars[INSIDE_CONTAINER_FILE_DIRECTORY],
                self.env_vars[CONTAINER_OUTPUT_LOG_FILE],
            )

            self.inside_config_file = os.path.join(
                self.env_vars[INSIDE_CONTAINER_FILE_MOUNT],
                self.env_vars[INSIDE_CONTAINER_FILE_DIRECTORY],
                self.env_vars[CONTAINER_INPUT_CONFIG_FILE_NAME],
            )
            self.inside_docker_share_dir = os.path.join(
                self.env_vars[INSIDE_CONTAINER_FILE_MOUNT],
                self.env_vars[INSIDE_CONTAINER_FILE_DIRECTORY]
            )
            try:
                with open(self.inside_config_file) as the_file:
                    self.configuration = json.load(the_file)
                    ACHelpers.resolve_secrets(self.configuration)
            except (IOError, ValueError) as e:
                s = "ACBase:  unable to load config.json  %s %s %s" % (type(e), e.args, str(e))
                self.valid_configuration = False
                ACLogger.log_and_print_error(s)

        if not (self.inside_docker_share_dir and os.path.isdir(self.inside_docker_share_dir)):
            s = "ACBase:  docker share dir does not exist %s" % self.inside_docker_share_dir
            self.valid_configuration = False
            ACLogger.log_and_print_error(s)
        ACLogger.log_and_print("Container init complete")
    ...
    def write_log(self):
        try:
            real_path_1 = os.path.realpath(AC_LOG_FILE_LOCATION)
            real_path_2 = os.path.realpath(self.inside_log_file)
            print(real_path_1, real_path_2)
            if real_path_1 != real_path_2:
                shutil.copyfile(AC_LOG_FILE_LOCATION, self.inside_log_file)
        except IOError:
            ACLogger.log_and_print_error(
                "ACBAse: unable to move log file from %s to %s" %
    ...
    def parse_command_line(self, parser):
        try:
            ...
            s = "ACBase:  During processing, caught an unknown exception:  %s %s" % (
                type(e), e.args
            )
            ACLogger.log_and_print_error(s)
            self.set_container_status(CONTAINER_STATUS_ERROR)
    ...
    def set_test_results(self, test_key, test_action, test_result, test_logic_str):
        ...
        if CONTAINER_TEST_DATA not in self.progress:
            self.progress[CONTAINER_TEST_DATA] = {}

        if test_key not in self.progress[CONTAINER_TEST_DATA]:
            self.progress[CONTAINER_TEST_DATA][test_key] = {}
        ...
        self.progress[CONTAINER_TEST_DATA][test_key][CONTAINER_TEST_DATETIME] = datetime.now(
        ).strftime("%Y-%m-%d %H:%M:%S.%f")
        return True

    def add_test_result_value(
        self, test_key, test_variable_value_type, test_variable_name, test_variable_value
    ):
        """
        Set the test value in the progress.json dictionary
        ...
        if test_variable_name is None:
            return False
        if (CONTAINER_TEST_DATA not in self.progress
                or test_key not in self.progress[CONTAINER_TEST_DATA]
                or isinstance(self.progress[CONTAINER_TEST_DATA][test_key], dict) is False):
            return False

        self.progress[CONTAINER_TEST_DATA][test_key][CONTAINER_TEST_VALUE_TYPE
                                                     ] = test_variable_value_type
        if CONTAINER_TEST_VALUE not in self.progress[CONTAINER_TEST_DATA][test_key]:
            self.progress[CONTAINER_TEST_DATA][test_key][CONTAINER_TEST_VALUE] = {}
        if test_variable_name not in self.progress[CONTAINER_TEST_DATA][test_key][
                CONTAINER_TEST_VALUE]:
            self.progress[CONTAINER_TEST_DATA][test_key][CONTAINER_TEST_VALUE] = {}
            self.progress[CONTAINER_TEST_DATA][test_key][CONTAINER_TEST_VALUE][test_variable_name
                                                                               ] = list()
        self.progress[CONTAINER_TEST_DATA][test_key][CONTAINER_TEST_VALUE][
            test_variable_name].append(test_variable_value)
    ...
    def print_container_state(self):
        ACLogger.log_and_print(self.__str__())
