...
import logging
from .ACSettings import K8S_SECRETS_BASE_PATH, AC_LOG_FILE_LOCATION, AC_LOGGING, AC_APPEND_MODE
import re
...
class ACLogger:
    ...
    @staticmethod
    def log_shell_output(output, prefix=None, print_lines=False):
        for output_line in output.strip().split("\n"):
            if prefix:
                output_line = prefix + ": " + output_line
            if print_lines:
                print(output_line)
            ACLogger().get_logger().info(output_line)

    @staticmethod
    def info(message):
        for line in str(message or "").split("\n"):
            ACLogger.log_and_print(line)

    @staticmethod
    def error(message):
        for line in str(message or "").split("\n"):
            ACLogger.log_and_print_error(line)

    @staticmethod
    def debug(message):
        for line in str(message or "").split("\n"):
            ACLogger().get_logger().debug(line)

    @staticmethod
    def warning(message):
        for line in str(message or "").split("\n"):
            ACLogger().get_logger().warning(line)

    @staticmethod
    def critical(message):
        for line in str(message or "").split("\n"):
            ACLogger().get_logger().critical(line)

    @staticmethod
    def setLevel(level):
        ACLogger().get_logger().setLevel(level)
    ...
    def init_from_string(self, jstr):
        try:
            ...
            ACLogger.log_and_print_error(
                "ACAppConfig: failed json.load check syntax %s. %s" % (jstr, e)
            )
            return False
        return self.validate_config()
                ...
                if statinfo.st_size > 0:
                    with open(file_json) as data_file:
                        try:
                            ...
                            ACLogger.log_and_print_error(
                                "ACAppConfig: failed json.load check syntax %s. %s" %
                                (file_json, e)
                            )
                            rv = False
                        else:
    ...
    def validate_config(self):
        for v in self._config_attributes:
            if v not in self._config_dict:
                ACLogger.log_and_print_error(
                    "ACAppConfig: failed to find %s in ACAppConfig.json" % v
                )
                return False
        return True
...
class ACHelpers:
    ...
    def create_timedelta_none(td_str):
        ...
        if td_str is None or isinstance(td_str, str) is False:
            ACLogger().get_logger().error(
                "create_timedelta: unable to get timedelta string %s" % td_str
            )
            return None
            ...
            try:
                ...
                if day_str is not None:
                    try:
                        ...
                        ACLogger().get_logger().error(
                            "create_timedelta: unable to get timedelta days %s" % td_str
                        )
                        return None
                    if len(hour_str.split(" ")) != 2:
                        ACLogger().get_logger().error(
                            "create_timedelta: unable to get timedelta space %s" % td_str
                        )
                        return None
                    else:
                        hs = hour_str.split(" ")[1]
                        try:
                            ...
                            ACLogger().get_logger().error(
                                "create_timedelta: unable to get timedelta hours %s" % td_str
                            )
                            return None
            if hours > 23:
        ...
        if have_error is True:
            ACLogger().get_logger().error(
                "create_timedelta: unable to parse timedelta string %s" % td_str
            )
            return None
        else:
            return t
    ...
    def resolve_secrets(input_dict):
        ...
        for key in input_dict.keys():
            ...
            if isinstance(val, str):
                ...
                input_dict[key] = [ACHelpers.resolve_vault_references(i) for i in val]

    @staticmethod
    def extract_vault_uris(some_text):
        uris = re.findall(r"(vault://[a-zA-Z_\-0-9/]+|#\{vault://[a-zA-Z_\-0-9/]+\})", some_text)
        return [u.replace("#{", "").replace("}", "") for u in uris]

    @staticmethod
    def replace_vault_in_text(some_text):
        uris = ACHelpers.extract_vault_uris(some_text)

        for uri in uris:
            value = ACHelpers.secret_resolve(uri)
            if value:
                some_text = some_text.replace("#{%s}" % uri, value).replace(uri, value)

        return some_text

    @staticmethod
    def resolve_vault_references(param):
        if isinstance(param, str):
            return ACHelpers.replace_vault_in_text(param)
        elif isinstance(param, list):
            return [ACHelpers.resolve_vault_references(i) for i in param]
        elif isinstance(param, dict):
            return {k: ACHelpers.resolve_vault_references(v) for k, v in param.items()}
        return param
    ...
    def secret_resolve(uri):
        if uri and isinstance(uri, str):
            if uri.startswith("vault://"):
                ...
                secret = ACHelpers.do_secret_resolve(env_var)
                if secret:
                    return secret
                return ACHelpers.do_secret_resolve_using_env_variables(env_var)
        return uri

    @staticmethod
    def do_secret_resolve(name):
        full_path = os.path.join(K8S_SECRETS_BASE_PATH, name)
    ...
    @staticmethod
    def do_secret_resolve_using_env_variables(env_var):
        try:
